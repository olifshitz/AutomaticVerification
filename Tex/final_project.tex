    ...
    \subsection{Read-Write Resource Agreement}
        ...
        \begin{figure}
            \begin{tikzpicture}
                ...
                \node[state, right of=isp] (o) {$owned$};
                \node[state, below of=o] (ow) {$owned-waiting$};   
                \draw 
                    (s) edge[sloped, above] node{$read-owned$} (i)
                    (s) edge[loop above] node{$idle$} (s)

                    (sw) edge[sloped, below] node{$read-owned$} (i)
                    (sw) edge[above] node{$response$} (s)
                    (sw) edge[loop above] node{$idle$} (sw)

                    (ssp) edge[sloped, below] node{$read-owned$} (isp)
                    (ssp) edge[loop above] node{$idle$} (ssp)

                    (o) edge[sloped, above] node{$read-owned$} (isp)
                    (o) edge[sloped, above] node{$read-shared$} (ssp)
                    (o) edge[loop right] node{$idle$} (o)

                    (ow) edge[sloped, above] node{$read-owned$} (i)
                    (ow) edge[sloped, above] node{$read-shared$} (sw)
                    (ow) edge[sloped, above] node{$response$} (o)
                    (ow) edge[loop below] node{$idle$} (ow)

                    (i) edge[sloped, above] node{$read-shared$} (sw)
