...
a,b = map(bddvar,'ab')
a_other,b_other=bddvar('a^'),bddvar('b^')
...
model_atomic_other = model_atomic.compose({msb[i]:msb_other[i] for i in range(model_index_length)})
model_atomic_other = model_atomic_other.compose({a:a_other,b:b_other})
